...
Decentralized full-text search is still an unsolved problem in peer-to-peer research. In this paper, we introduce Sweep, a fully decentralized full-text search engine, that takes significant steps towards providing reliable, low-latency, and accurate search results. Sweep is built on Apache Lucene and each peer maintains a replica of a bounded-size subset
of the whole search index. Full-text search involves a fan-out search
to each shard, with latency variance reduction techniques to
help achieve low response times. Our main contributions include a novel gossip-based protocol for fast and efficient replication of the search index and a protocol for the automated sharding of the search index. Our approach is based on a low overhead gossip-based leader selection algorithm within each shard, whose cost is independent of the number of peers. For each shard, peers add new index entries to the leader group, and peers securely pull updates within their shard using a Gradient topology that ensures efficient dissemination of updates in log(N) hops within the shard. We show in simulation the viability of our approach and its robustness to failures and flash crowds.
...
In section[reference] we introduced the concept of the dense identifier space created by the current identifier structure. In this section we will explain the use of these dense spaces in event of network partition merge. 
...
%\bibliographystyle{•}bibliographystyle{myIEEEtran}
\appendix
